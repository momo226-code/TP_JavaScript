<template>
    <div id="app">
        ...
        <div class="personne" v-for="p in filteredPersonnes" :key="p.login.uuid" :class="{ misEnForme: miseEnForme }">

                <div v-if="(((homme && p.name.title === 'Mr') || (femme && p.name.title === 'Mrs')) 
&& (p.name.last.indexOf(filtreNom) >= 0 || filtreNom === ''))">

        

        <!-- Affichage des photos de la personne -->
        <img v-if="photoSize === '1'" :src="p.picture.thumbnail" alt="Photo Thumbnail" />
        <img v-if="photoSize === '2'":src="p.picture.medium" alt="Photo Medium" />
        <img v-if="photoSize === '3'":src="p.picture.large" alt="Photo Large" />

        <!-- Nom et prénom de la personne -->
        {{ p.name.title }} {{ p.name.first }} {{ p.name.last }}<br>
        
        <button @click="displayContact(p)">Afficher le contact</button>

        <!-- Affichage du téléphone si activé -->
        <div v-if="afficherTel || p.afficherTel">
          <div>{{ p.phone }}</div>
          <div>{{ p.email }}</div>
        </div>
...
export default{
    ...
    return {
        ...
        personnes :[
        {
          ...
          location: {
            ...
            country: 'Canada',
            postcode: 'N7H 5G1',
            coordinates: { latitude: '-67.5091', longitude: '66.5485' },
          ...
          registered: { date: '2010-12-21T12:23:45.450Z', age: 11 },
          phone: '366-033-8063',
          cell: '205-901-7452',
          id: { name: '', value: null },
          picture: {
          ...
          nat: 'CA',
            afficherTel: false, // Ajout de la propriété showContact
        },
        {
          gender: 'male',
          ...
          location: {
            ...
            country: 'Brazil',
            postcode: 78657,
            coordinates: { latitude: '-86.0633', longitude: '45.8313' },
          ...
          registered: { date: '2002-11-14T17:14:04.368Z', age: 19 },
          phone: '(53) 5553-2892',
          cell: '(04) 3893-4079',
          id: { name: '', value: null },
          picture: {
        ...
        },
        {
          ...
          location: {
            ...
            country: 'Switzerland',
            postcode: 9476,
            coordinates: { latitude: '37.5062', longitude: '-115.1853' },
          ...
          registered: { date: '2003-01-18T13:41:59.518Z', age: 18 },
          phone: '079 493 00 77',
          cell: '079 283 03 14',
          id: { name: 'AVS', value: '756.3512.8312.75' },
          picture: {
  ...
  computed: {
  filteredPersonnes() {
    let result = this.personnes.filter((p) =>
      (
        (!this.homme && !this.femme) || 
  ...
  methods: {
    displayContact(person) {
      // Alterne l'état de showContact
      person.afficherTel = !person.afficherTel;
    },
  },
} ;
